...
class Tools:
    ...
    async def compress(self, dl, out):
        cmd = f'''{Var.FFMPEG} -i """{dl}""" -metadata "Encoded By"="https://t.me/movierulzking" -preset ultrafast -c:v libx265 -crf {Var.CRF} -map 0:v -c:a aac -map 0:a -c:s copy -map 0:s? """{out}""" -y'''
        process = await asyncio.create_subprocess_shell(
